...
/************************************* MACRO per la gestione dell'array d'appoggio */
/***********************************************************************************/
#define free_array(WATOR)                       \
    /* Dealloca tutto l'array d'appoggio */     \
    if (array != NULL)                          \
    {                                           \
...
#define create_array(WATOR)                                                         \
    /* Alloca l'array per il conteggio dei turni */                                 \
    if ((array = (char **) malloc(sizeof(char *) * WATOR->plan->nrow)) == NULL)     \
    {                                                                               \
        /* errno settato da malloc() in caso di errori. */                          \
        free_wator(WATOR);                                                          \
    ...
    for (i = 0; i < WATOR->plan->nrow; ++i)                                         \
    {                                                                               \
        if ((array[i] = (char *) malloc(sizeof(char) * WATOR->plan->ncol)) == NULL) \
        {                                                                           \
            /* In caso di errori, libera array e wator e vai all'errore */          \
            free(array);                                                            \
            free_wator(WATOR);                                                      \
            error_handle(handle_new_wator, NULL);                                   \
        }                                                                           \
                                                                                    \
        /* Inizializzo l'i-esimo array a zero */                                    \
        memset(array[i], 0, WATOR->plan->ncol);                                     \
    }                                                                            

#define initialize_array(WATOR)                     \
...
#define error_handle(HANDLE, RET_VAL) \
    HANDLE();                         \
    return RET_VAL;

/* MACRO che setta il valore di errno e ritorna RET_VAL */
#define errno_set(VALUE, RET_VAL) \
    errno = VALUE;                \
    return RET_VAL;

/* Handler per l'errore ERANGE in load_planet */
#define handle_load_planet() \
    free_planet(result);     \
    errno = ERANGE;

/* Handler per la chiusura dei file in new_wator */
#define handle_new_wator() \
    fclose(conf);          \
    fclose(plan);
...
/* Struttura punto bidimensionale, da utilizzare
   nelle regole di aggiornamento.
 */
